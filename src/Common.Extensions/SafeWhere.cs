...
public static partial class Extensions
{
    ...
    /// <returns>A collection of elements that satisfy the predicate.</returns>
    [DebuggerStepThrough]
    public static ICollection<TSource> SafeWhere<TSource>(this ICollection<TSource> source, Func<TSource, bool> predicate)
    {
    ...
    [DebuggerStepThrough]
    public static IDictionary<TKey, TValue> SafeWhere<TKey, TValue>(this IDictionary<TKey, TValue> source, Func<KeyValuePair<TKey, TValue>, bool> predicate)
    {
        return (source ?? new Dictionary<TKey, TValue>()).Where(predicate).ToDictionary(kvp => kvp.Key, kvp => kvp.Value);
    }
}
